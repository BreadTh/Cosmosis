...
public interface ICosmosisQueryPostOrdered<T>
{
    ...
    ICosmosisQueryPostOrdered<T> OrderBy<TKey>(Expression<Func<T, TKey>> keySelector);
    [Obsolete("OrderBy has already been applied.", true)]
    ICosmosisQueryPostOrdered<T> OrderByDescending<TKey>(Expression<Func<T, TKey>> keySelector);
    [Obsolete("ThenBy must come immediately after OrderBy. Rearrange to .OrderBy(...).ThenBy(...) before .Where(...).", true)]
    ICosmosisQueryPostOrdered<T> ThenBy<TKey>(Expression<Func<T, TKey>> keySelector);
    [Obsolete("ThenByDescending must come immediately after OrderBy. Rearrange to .OrderBy(...).ThenByDescending(...) before .Where(...).", true)]
    ICosmosisQueryPostOrdered<T> ThenByDescending<TKey>(Expression<Func<T, TKey>> keySelector);
