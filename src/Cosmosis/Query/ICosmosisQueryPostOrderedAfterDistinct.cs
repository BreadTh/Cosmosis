...
public interface ICosmosisQueryPostOrderedAfterDistinct<T>
{
    ICosmosisQueryPostOrderedAfterDistinct<T> Where(Expression<Func<T, bool>> predicate);
    ...
    [Obsolete("Distinct has already been applied.", true)]
    ICosmosisQueryPostOrderedAfterDistinct<T> Distinct();
    [Obsolete("OrderBy has already been applied.", true)]
    ICosmosisQueryPostOrderedAfterDistinct<T> OrderBy<TKey>(Expression<Func<T, TKey>> keySelector);
    [Obsolete("OrderBy has already been applied.", true)]
    ICosmosisQueryPostOrderedAfterDistinct<T> OrderByDescending<TKey>(Expression<Func<T, TKey>> keySelector);
    [Obsolete("ThenBy must come immediately after OrderBy. Rearrange to .OrderBy(...).ThenBy(...) before .Where(...).", true)]
    ICosmosisQueryPostOrderedAfterDistinct<T> ThenBy<TKey>(Expression<Func<T, TKey>> keySelector);
    [Obsolete("ThenByDescending must come immediately after OrderBy. Rearrange to .OrderBy(...).ThenByDescending(...) before .Where(...).", true)]
    ICosmosisQueryPostOrderedAfterDistinct<T> ThenByDescending<TKey>(Expression<Func<T, TKey>> keySelector);
