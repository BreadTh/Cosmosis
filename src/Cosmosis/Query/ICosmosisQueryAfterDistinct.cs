...
public interface ICosmosisQueryAfterDistinct<T>
{
    ICosmosisQueryAfterDistinct<T> Where(Expression<Func<T, bool>> predicate);
    ...
    ICosmosisQueryAfterDistinct<T> ThenBy<TKey>(Expression<Func<T, TKey>> keySelector);
    [Obsolete("ThenByDescending requires OrderBy first. Rearrange to .OrderBy(...).ThenByDescending(...).", true)]
    ICosmosisQueryAfterDistinct<T> ThenByDescending<TKey>(Expression<Func<T, TKey>> keySelector);
